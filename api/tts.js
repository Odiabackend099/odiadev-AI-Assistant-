...
async function tryEndpoints(base, userKey, text, voice){
  ...
  for (const c of candidates){
    ...
    if (r.ok) {
      ...
      return { ok: true, buf, type: r.headers.get('content-type') || 'audio/mpeg', used: c.url };
    }
...
module.exports = async function handler(req, res){
  ...
  if(req.method === 'GET'){
    ...
    try{ const j = JSON.parse(body||'{}'); text = (j.text||'').trim(); voice = (j.voice||voice).trim(); }catch{}
  }
  ...
  if(!base){
    ...
    return res.end(JSON.stringify({ fallback: true, note: 'No TTS service configured' }));
  }

  try{
    const tried = await tryEndpoints(base, userKey, text, voice);
    if(tried.ok){
      res.setHeader('Content-Type', tried.type);
      res.setHeader('Cache-Control','no-store');
      return res.end(tried.buf);
    }
    res.statusCode=502;
    res.end('TTS upstream unavailable');
  }catch(e){
    res.statusCode=500;
